...
FROM movie
WHERE id NOT IN (SELECT movie_id FROM genre GROUP BY movie_id HAVING COUNT(*) > 1) ;
...
+---------------+-------------------+---------------------+*/
-- Type your code below:
SELECT title, avg_rating, genre FROM genre 
...
SELECT names.name AS actor_name, COUNT(*) AS movie_count
FROM names
...
WHERE category = 'Actor' AND ratings.median_rating >= 8
GROUP BY names.name
ORDER BY movie_count DESC
LIMIT 2;
...
JOIN (SELECT @row_number := 0) AS init_user_params
GROUP BY production_company
...
WITH cte AS (
SELECT title, avg_rating, total_votes,
ROUND(avg_rating * total_votes / total_votes, 2) AS weighted_avg_rating
FROM movie m
INNER JOIN ratings r ON m.id = r.movie_id
INNER JOIN genre g ON m.id= g.movie_id
WHERE genre = 'thriller'
)
SELECT title, weighted_avg_rating,
CASE
WHEN weighted_avg_rating > 8 THEN 'Superhit movie'
WHEN weighted_avg_rating BETWEEN 7 AND 8 THEN 'Hit movie'
WHEN weighted_avg_rating BETWEEN 5 AND 7 THEN 'One-time-watch movie'
ELSE 'Flop movie'
...
FROM movie 
JOIN ratings ON movie.id = ratings.movie_id
WHERE languages LIKE '%,%' AND median_rating >= 8 and production_company is NOT NULL AND POSITION(',' IN languages)>0
GROUP BY production_company
HAVING COUNT(movie.id) > 0
...
SELECT 
    ...
    Round(AVG(r.avg_rating),1) as actress_avg_rating,
    ROW_NUMBER() OVER (ORDER BY AVG(r.avg_rating) DESC) as actress_rank
FROM 
    names n 
